...
<html lang="en">
	...
	<div class = "album_tables" >
		...
		$statement = $sleds_database -> prepare( "SELECT playlist.id, playlist.name, cluster.name FROM playlist JOIN cluster ON playlist.id_cluster=cluster.id JOIN relation_user_cluster ON relation_user_cluster.id_cluster=cluster.id WHERE relation_user_cluster.id_user=?" );
		$statement -> bind_param( "i", $_SESSION[ "user_id" ] );
		$statement -> execute();
		$playlists_result = $statement -> get_result();

		// Go throught all the playlists
		while ( $playlist = $playlists_result -> fetch_assoc() )
		{
			// Print a bar to hide the album
			?>
			<div class = "table_hide_show">
				<a class = "hide_show_text"><?php echo $playlist[ "name" ]; ?></a>
				<a class = "material-icons hide_show" ><button class = "hide_show_button" id = "<?php echo $playlist[ "name" ]; ?>hide_show_icon" onclick="hide_show_table('<?php echo $playlist[ 'name' ]; ?>')">expand_more</button></a>
			</div>
			<?php
			// Print the head of the table
			?>
			<div id="<?php echo $playlist[ 'name' ]; ?>_table">
			...
			$statement = $sleds_database -> prepare( "SELECT animation.id, animation.name, animation.leds_number, animation.phases, animation.delay, animation.repeat FROM animation WHERE animation.id_playlist=?" );
			$statement -> bind_param( "i", $playlist[ "id" ] );
			$statement -> execute();
